...
# Loading Screen at the beggining at the code:
Font = pygame.font.Font('PIXELADE.TTF', 55)
...
# Fonts:
RevealFont = pygame.font.Font('PIXELADE.TTF', 65)
FontDifficulty = pygame.font.Font('PIXELADE.TTF', 65)
InstructionsFont = pygame.font.Font('PIXELADE.TTF', 40)
FullWordGuessFont  = pygame.font.Font('PIXELADE.TTF', 48)
CountdownFont = pygame.font.Font('PIXELADE.TTF', 100)

# Load Images:
RevealSignPNG = pygame.image.load("RevealSignBoard.png")
KeybaordSignPNG = pygame.image.load("KeyboardSignBoard.png")
CountDownBlackOutPNG = pygame.image.load("CountDownBlackOut.png")
FullWordGuessPopUpPNG = pygame.image.load("FullWordGuessPopUp.png")

# Load Sound Effects and Music files:
mp3_BigButtonPressFX = pygame.mixer.Sound("BigButtonPressFX.mp3")
mp3_CountDownSFX = pygame.mixer.Sound("CountDownSFX.mp3")
mp3_WinFanfareFX = pygame.mixer.Sound("Indy's Fanfare.mp3")
mp3_LooseLifeFX = pygame.mixer.Sound("LooseLifeFX.mp3")
mp3_SmallButtonPressFX = pygame.mixer.Sound("LightSwitchFX.mp3")
mp3_KeyboardTypingFX = pygame.mixer.Sound("KeyboardTypingFX.mp3")
mp3_BlcokedFX = pygame.mixer.Sound("BlockedFX.mp3")
...
class HangingMan:
    def __init__(self):
        ...
        self.image = []
        for x in range(0,8):
            self.image.append(pygame.image.load('Error1.'+str(x)+ 'HangmanPictures.png'))


    def draw(self, errors):
        WINDOW.blit(self.image[errors], (self.x, self.y))

# Initialize HangminMan class instance
...
class TitleScreen: 
    def __init__(self, name):
        ...
        self.showing = False
        self.buttons = []
        self.music = "Indiana Jones Main Theme.mp3" 
    ...
    def Run(self):
                ...
                if event.type == pygame.MOUSEBUTTONUP: #Manage events when ButtonUp for better interface experience
                    ...
                    for button in self.buttons:
                        if button.isClicked(mousePos):
                            ...
                            elif button == MusicOFFButtonTittleScreen and MusicOFFButtonTittleScreen.show:
                                MusicONButtonTittleScreen.show, MusicONButtonTittleScreen.hide = True, False
                                button.show = False
                                button.hide = True
                                bMusicOn = True
                                fnPlayMusic(self.music)

                            elif button == MusicONButtonTittleScreen and MusicONButtonTittleScreen.show:
                                MusicOFFButtonTittleScreen.show, MusicOFFButtonTittleScreen.hide = True, False
                                button.show = False
                                button.hide = True
                                bMusicOn = False
                                pygame.mixer.music.stop()

                            elif button == SoundEffectsOFFButtonTittleScreen and SoundEffectsOFFButtonTittleScreen.show:
                                SoundEffectsONButtonTittleScreen.show, SoundEffectsONButtonTittleScreen.hide = True, False
                                button.show = False
                                button.hide = True
                                bSoundEffectsOn = True

                            elif button == SoundEffectsONButtonTittleScreen and SoundEffectsONButtonTittleScreen.show:
                                SoundEffectsOFFButtonTittleScreen.show, SoundEffectsOFFButtonTittleScreen.hide = True, False
                                button.show = False
                                button.hide = True
        ...
        pygame.mixer.music.stop() #stop music when exit tittle screen
...
class InstructionsScreen: 
    def __init__(self, name):
        ...
        self.buttons = []
        self.AllFrames = [pygame.image.load(f"{name}1.1.png"), pygame.image.load(f"{name}1.2.png"), pygame.image.load(f"{name}1.3.png"), pygame.image.load(f"{name}1.4.png"), pygame.image.load(f"{name}1.5.png"), pygame.image.load(f"{name}1.6.png")]
        self.AllScaledFrames = []
        self.AllFramesScaledRectangles = []
        self.music = "InstructionsScreen-Map.mp3" 

        #   resize images to fit screen dimentions: 
                    ...
                    if Transition_2_Over and pygame.time.get_ticks() - fTimeSinceTransition2 > 400 :
                        ...
                        for button in self.buttons:
                            if button.isClicked(mousePos):
                                ...
                                RunGame = False 
                                # Returns string deppending on the difficulty selected
...
class GameScreen:
    def __init__(self, name, Difficulty):
        ...
        self.music = "GameplayDesperateSituation.mp3"
        self.sAnswer = ''
        self.LettersToGuess = []
        self.LettersGuessed = []
        self.iScore = 1000
        ...
        #   resize image to fit screen dimentions: 
        original_width, original_height = self.image.get_size()
        scale_factor_width = WIDTH / original_width
        scale_factor_height = HEIGHT / original_height
        scale_factor = min(scale_factor_width, scale_factor_height)
    ...
    def fnDefineNewAnswer(self):
        ...
        if self.difficulty == 'Easy': # more than 10 characters long
            ...
            self.RevealFont = pygame.font.Font('PIXELADE.TTF', 45)
            ...
            self.iRevealYPosition = 134
            self.RevealFont = pygame.font.Font('PIXELADE.TTF', 85)
        ...
        # print(f'the new word is {self.sAnswer}') <-- UNCOMENT FOR CHEATING
    ...
    def draw(self):
        ...
        WINDOW.blit(KeybaordSignPNG, (420, 88))
        ...
        TextSurfaceScore = Font.render((f"Score: {str(self.iScore)}"), True, WHITE) # Draw Errors
    ...
    def fnDrawInCountdown(self, sCountdown):
        ...
        TextSurfaceCountown = CountdownFont.render(str(sCountdown), True, WHITE) 
        text_width, text_height = TextSurfaceCountown.get_size()
        x_centered = WIDTH//2 - text_width // 2 
        WINDOW.blit(TextSurfaceCountown, (x_centered, HEIGHT//2 - 40))
    ...
    def fnCountdown(self):
        ...
        while Run:
            sCountdown = '3'
            fCountownTime = (pygame.time.get_ticks()- self.startTime)*.001
            if fCountownTime > 3.5:
                Run = False
            elif fCountownTime > 3.1:
                sCountdown = 'Go!'
            elif fCountownTime > 2.1:
                sCountdown = '1'
            elif fCountownTime > 1.1:
                sCountdown = '2'
            else:
                sCountdown = '3'
    ...
    def Run(self, difficulty):
        ...
        while RunGame == True:
            ...
            for event in pygame.event.get(): # Huge event pump
                if event.type == pygame.QUIT:
                ...
                elif event.type == pygame.KEYDOWN:  # Takes in input form keyboard
                    if not self.GuessingFullWord:   # Normal keyboard input
                        ...
                        if letter.isalpha() and letter.lower() not in self.LettersGuessed:
                            ...
                            fnPlaySoundEffects(mp3_BlcokedFX)
            ...
            self.draw() #draw everything (gallow, reveal, errors, y screen and full word guess menu)
...
class VictoryScreen: 
    def __init__(self):
        ...
        self.image = pygame.image.load("VictoryScreen.png")
        self.buttons = []
    ...
    def draw(self):
        ...
        TextSurface = CountdownFont.render(self.BigText, True, WHITE) #draw Win/loose big message
    ...
    def Run(self, score, sAnswer):
        ...
        # Create custom text deppening on the score
        if score > 0: 
            fnPlaySoundEffects(mp3_WinFanfareFX)
            self.music = "VictoryScreen-Indy and Sophia's Kiss.mp3"
            self.BigText = 'YOU WIN!'
            self.mediumText = f"Your score was: {score}"
        else :
            self.music = "LooseScreen-Ominous Feeling.mp3"
            self.BigText = "YOU LOOSE!"
            self.mediumText = f"The word was: {sAnswer}"
            if len(sAnswer) > 11:
                self.mediumTextFont = pygame.font.Font('PIXELADE.TTF', 35)
...
class Button:
    Buttons = []
    def __init__(self, x, y, width, height, text, BelongingScreen, purpose, show, button_image_normal, button_image_hovered, button_image_pressed, textcolor, SoundEffect, font =pygame.font.Font('PIXELADE.TTF', 55) ):
        ...
        # rezise images to match the inputed rectangles dimentions
...
for letter in AlphabetList: #creates buttons for the keyboard
    ...
    letter = letter.upper()
    KeyboardLockPNGImagesDictionary[letter] = pygame.image.load(f"Normal{letter}.png")
    width, height = KeyboardLockPNGImagesDictionary[letter].get_width(), KeyboardLockPNGImagesDictionary[letter].get_height()
    ImageAspectRatio = width/height

    KeyboardPressPNGImagesDictionary[letter] = pygame.image.load(f"Hover{letter}.png")
    KeyboardReleasePNGImagesDictionary[letter] = pygame.image.load(f"Press{letter}.png")
    ...
    iImageXPosition += ImageAspectRatio*KeyBoardButtonsSizeHeight + 20 # move each letter 20 appart from each other


PNGPlayButtonLock = pygame.image.load("play01.png")
PNGPlayButtonPress = pygame.image.load("play02.png")
PNGPlayButtonRelease = pygame.image.load("play03.png")
...
MusicAndSoundButtonsSize = 100 #width For tittle screen buttons
...
PNGMusicONButtonLock = pygame.image.load("MusicButtonON.png")
PNGMusicONButtonPress = pygame.image.load("MusicButtonON-Hovered.png")
PNGMusicONButtonRelease = pygame.image.load("MusicButtonON-Clicked.png")
PNGMusicOFFButtonLock = pygame.image.load("MusicButtonOFF.png")
PNGMusicOFFButtonPress = pygame.image.load("MusicButtonOFF-Hovered.png")
PNGMusicOFFButtonRelease = pygame.image.load("MusicButtonOFF-Clicked.png")
...
MusicONButtonTittleScreen = Button(770, 550, MusicAndSoundButtonsSize  , MusicAndSoundButtonsSize*MusicONButtonRatio, '', TitleScreen_Hangman, 'MusicOn', True, PNGMusicONButtonLock, PNGMusicONButtonPress, PNGMusicONButtonRelease, BLACK, mp3_SmallButtonPressFX)
MusicOFFButtonTittleScreen = Button(770, 550, MusicAndSoundButtonsSize  , MusicAndSoundButtonsSize*MusicOFFButtonRatio, '', TitleScreen_Hangman, 'MusicOff', False,PNGMusicOFFButtonLock, PNGMusicOFFButtonPress, PNGMusicOFFButtonRelease, BLACK, mp3_SmallButtonPressFX)
...
PNGSoundEffectsONButtonLock = pygame.image.load("SoundEffectsButtonON.png")
PNGSoundEffectsONButtonPress = pygame.image.load("SoundEffectsButtonON-Hovered.png")
PNGSoundEffectsONButtonRelease = pygame.image.load("SoundEffectsButtonON-Clicked.png")
PNGSoundEffectsOFFButtonLock = pygame.image.load("SoundEffectsButtonOFF.png")
PNGSoundEffectsOFFButtonPress = pygame.image.load("SoundEffectsButtonOFF-Hovered.png")
PNGSoundEffectsOFFButtonRelease = pygame.image.load("SoundEffectsButtonOFF-Clicked.png")

SoundEffectsONButtonTittleScreen = Button(85, 550, MusicAndSoundButtonsSize  , MusicAndSoundButtonsSize*MusicONButtonRatio, '', TitleScreen_Hangman, 'SoundEffectsOn', True, PNGSoundEffectsONButtonLock, PNGSoundEffectsONButtonPress, PNGSoundEffectsONButtonRelease, BLACK, mp3_SmallButtonPressFX)
SoundEffectsOFFButtonTittleScreen = Button(85, 550, MusicAndSoundButtonsSize  , MusicAndSoundButtonsSize*MusicOFFButtonRatio, '', TitleScreen_Hangman, 'SoundEffectsOff', False,PNGSoundEffectsOFFButtonLock, PNGSoundEffectsOFFButtonPress, PNGSoundEffectsOFFButtonRelease, BLACK, mp3_SmallButtonPressFX)
...
# More buttons: 
PNGFullWordGuessButtonLock = pygame.image.load("FullWordGuessButtonNormal.png")
PNGFullWordGuessButtonPress = pygame.image.load("FullWordGuessButtonHovered.png")
PNGFullWordGuessButtonRelease = pygame.image.load("FullWordGuessButtonPressed.png")
...
PNGDifficultySelectionButtonLock = pygame.image.load("UI_Flat_Select_01a1.png")
PNGDifficultySelectionButtonPress = pygame.image.load("UI_Flat_Select_01a2.png")
PNGDifficultySelectionButtonRelease = pygame.image.load("UI_Flat_Select_01a3.png")
...
PNGPlayVictoryScreenLock = pygame.image.load("VictoryScreenButtonNormal.png")
PNGPlayVictoryScreenPress = pygame.image.load("VictoryScreenButtonHovered.png")
PNGPlayVictoryScreenRelease = pygame.image.load("VictoryScreenButtonPressed.png")
...
BackToMenuButton = Button(705 , 259, 181, 67, 'Back to Menu', Victory_Screen, 'BackToMenu', True, PNGPlayVictoryScreenLock, PNGPlayVictoryScreenPress, PNGPlayVictoryScreenRelease, WHITE, mp3_SmallButtonPressFX, pygame.font.Font('PIXELADE.TTF', 35) )
...
def fnPlayMusic(SoundFile):
    # Takes in soundfile name (string) argument and plays the song indifenitely
    if bMusicOn: 
...
def TitleAndInstructions():
    ...
    TitleScreen.Run(TitleScreen_Hangman)
    return InstructionsScreen.Run(Instructions_Screen)

def GameAndVictory(Difficulty):
    # takes in string of difficulty and runs the game with that difficulty indifenitely. If player returns to title screen, returns false.
    iScore, sAnswer = GameScreen.Run(GamePlay_Screen, Difficulty)
...
def RunFullGame():
    # Runs game indifenitely until window is exited.
    Run = True
